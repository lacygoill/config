...
" colored text; if it's not the case, repeat `:edit` + `50@:`.
...
function! s:get_256color(colors) abort
    if len(a:colors) >= 2 " May be fewer while editing
        let [_five, color] = a:colors->remove(0, 1)
        if _five != '5' || color == ''
            return -1
        endif
        return str2nr(color)
...
function! s:preview_color(linenr) abort
    ...
    let hi_attrs = []
    while len(colors) > 0
        let item = colors->remove(0)->str2nr()
        if item == 1
    ...
    if s:currentmatch !~# '\/\\_s\\zs' .. colordef .. '\\ze\\_s\/'
        ...
        if !empty(hi_attrs)
            if has('gui_running')
                let hi_attrs_str = ' gui=' .. hi_attrs->join(',')
            else
                let hi_attrs_str = ' cterm=' .. hi_attrs->join(',')
            endif
        endif
...
if has('gui_running') || &t_Co != ''
    ...
    augroup MyDircolorsSyntax
        ...
        autocmd CursorHold <buffer>
            \   if b:current_syntax !=# 'dircolors'
            \ |     execute 'autocmd! MyDircolorsSyntax'
            \ |     augroup! MyDircolorsSyntax
            \ | endif
    augroup END
endif

...
export def OpenTerm(dir: string)
    ...
    if IS_TMUX
        silent system('tmux split-window -h -c ' .. shellescape(dir))
    elseif IS_X_RUNNING
...
def ErrorMeaning()
    ...
    var makeprg: string = bufnr('#')->getbufvar('&makeprg')
    var url: string
...
export def WebpageRead(url: string) #{{{1
    if !executable('w3m')
        echo 'w3m is not installed'
        return
    endif
    var tmp_file: string = $'{tempname()}/webpage'
    execute $'tabedit {tmp_file}'
    # Alternative shell command:  `$ lynx -dump -width=1000 <URL>`
    execute $'silent read !w3m -cols 1000 {shellescape(url, true)}'
    #                                ^--^
    #                                high number to be sure that `lynx` won't break long lines of code
    &l:buftype = 'nofile'
    &l:buflisted = false
    &l:swapfile = false
    &l:wrap = false

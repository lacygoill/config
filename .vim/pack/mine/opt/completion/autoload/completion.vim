...
const EXIT_CTRL_X: string = "\<C-X>\<C-Z>"
...
# The  filename characters  afterward  are  optional, because  we  could try  to
# complete `some_dir/` or just `~`.
#}}}
const YES_YOU_CAN: func = (_): bool => true
const MC_CONDITIONS: dict<func(string): bool> = {
    C-p: (_): bool => manual && util.CustomIsk('-'),
    dict: (_): bool => manual && util.SetupDict(),
    digr: (_): bool => manual && get(g:, 'loaded_unicodePlugin', false),
    file: (t: string): bool => t =~ '[/~]\f*$',

    # `keyn`  is our  first  default method  in `MC_CHAIN`  which  is used  to
    # complete code/prose.  And in practice, we want the ability to complete a
    # word containing a hyphen (e.g. `set-environment` in a bash script).
    keyn: (_): bool => manual && util.CustomIsk('-'),

    omni: (_): bool => !empty(&l:omnifunc) && &filetype != 'markdown',
    spell: (_): bool => &l:spell && !empty(&l:spelllang),
    tags: (_): bool => manual
                && !tagfiles()->empty()
                && util.CustomIsk('-' .. (&filetype == 'vim' ? ':<' : '')),
...
# Interface {{{1
export def Complete(arg_dir: number): string #{{{2
    word = getline('.')
        ->strpart(0, col('.') - 1)
...
export def Cycle(arg_dir: number): string #{{{2
    cycling = true
    manual = true
    dir = arg_dir
    i_history = []
...
# This  flag lets  `VerifyCompletion()` choose  between  acting on  the menu  if
# there's one, or trying another method.
#
# It's reset to 0 at the beginning of `ActOnPumvisible()`.
#}}}

export def SnippetOrComplete(arg_dir: number) #{{{2
...
export def TabComplete(arg_dir: number): string #{{{2
    manual = true
    orig_line = getline('.')
    return Complete(arg_dir)
enddef
...
export def VerifyCompletion(): string #{{{2
    return pumvisible
        ?     ActOnPumvisible()
        :     NextMethod()
enddef
...
export def RestoreBase() #{{{2
    if orig_line != ''
        setline('.', orig_line)
        augroup CompletionUnletOrigLine
            autocmd!
...
export def ThesaurusFunc(findstart: bool, base: string): any #{{{2
    if findstart
        return searchpos('\<', 'bnW', line('.'))[1] - 1
    endif

    silent var lines: list<string> = systemlist('aiksaurus ' .. shellescape(base))
...
def ActOnPumvisible(): string #{{{2
    ...
    #
    # If the method is 'spell', don't do anything either.
    #
    # Why?
    ...
    #
    # Otherwise, autocompletion is off, and the current method is not 'spell'.
    ...
    return auto || get(methods, i, '') == 'spell'
        ?     ''
...
def ActOnTextchanged() #{{{2
    ...
    if completedone
        ...
        # Inside `TabComplete()`, it's set to 1.
        # Inside `EnableAuto()`, it's set to 0.
        #
        ...
        # Why `filepath.Complete()`? {{{
        #
        # Usually, when a completion has been done, we don't want autocompletion
        # to be invoked again right afterward.
        #
...
# calls `Complete(1)`.  Similar to pressing Tab.
#}}}
...
def NextMethod(): string #{{{2
    ...
    # What's the meaning of: `&& i_history->index(i) == -1`?{{{
    #
    ...
    #}}}
    if i != -1 && i != N && i_history->index(i) == -1
        ...
        #
        # 2 - Store the state of the menu in `pumvisible` through `MenuIsUp()`.
        #
        # 3 - call `VerifyCompletion()` through `<Plug>(MC_next_method)`
        #}}}
        ...
        # MRE:
        #
        # First temporarily disable `util.SetupDict()` in `MC_CONDITIONS`:
        #
        #     dict: (_): bool => manual && util.SetupDict(),
        #     →
        #     dict: (_): bool => manual,
...
#
#    - `VerifyCompletion()`    after a failed completion
#    - `Complete()`            1st attempt to complete (auto / manual)
#    - `Cycle()`               when we cycle
#}}}
# Purpose: {{{
#

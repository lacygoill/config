...
import autoload '../autoload/completion.vim'
import autoload '../autoload/completion/custom.vim'
import autoload '../autoload/completion/spell.vim'
import autoload '../autoload/completion/util.vim'
...
inoremap  <silent><unique> <Tab> <ScriptCmd>completion.SnippetOrComplete(1)<CR>
inoremap  <silent><unique> <S-Tab> <ScriptCmd>completion.SnippetOrComplete(-1)<CR>
...
snoremap <unique>   <Tab> <C-\><C-N><ScriptCmd>UltiSnips#JumpForwards()<CR>
snoremap <unique> <S-Tab> <C-\><C-N><ScriptCmd>UltiSnips#JumpBackwards()<CR>
...
# TODO: Document why we need `completion.RestoreBase()`.
# Hint: it's due to `longest` being in `'completeopt'`.
# Also, document why we don't invoke `completion.RestoreBase()` in `<Plug>(MC_c-e)`.
# Hint: it would break the dot command too frequently (as soon as we cycle).
inoremap <C-Q> <C-E><ScriptCmd>completion.RestoreBase()<CR>

# completion stop {{{2

# We rely on the  Vim command `<C-X><C-Z>` to exit the  pum without changing the
# last inserted text (see `:help compl-stop`).   But it might have been remapped
# by another plugin.  We don't want that to happen.
autocmd VimEnter * silent! iunmap <C-X><C-Z>
...
nnoremap <unique> [oM <ScriptCmd>completion.EnableAuto()<CR>
nnoremap <unique> ]oM <ScriptCmd>completion.DisableAuto()<CR>
nnoremap <unique> coM <ScriptCmd>completion.ToggleAuto()<CR>

# improved default methods {{{2
# C-p         &friends {{{3

# What's the purpose of `util.CustomIsk()`?{{{
#
...
#}}}
inoremap <unique> <C-X><C-N> <ScriptCmd>util.CustomIsk('-')<CR><C-X><C-N>
inoremap <unique> <C-X><C-P> <ScriptCmd>util.CustomIsk('-')<CR><C-X><C-P>
...
#}}}
#                                                              │
inoremap <unique> <C-X><C-]> <ScriptCmd>util.CustomIsk('-' .. (&filetype == 'vim' ? ':<' : ''))<CR><C-X><C-]>

# C-x C-k     dictionary {{{3

inoremap <unique> <C-X><C-K> <ScriptCmd>util.SetupDict()<CR><C-X><C-K>

# C-x C-s     fix Spelling error {{{3

inoremap <expr><unique> <C-X><C-S> spell.Fix()
...
inoremap <unique> <C-X>s <ScriptCmd>mode()->custom.Signature()<CR>
cnoremap <unique> <C-X>s <C-\>e <SID>custom.Signature(mode())<CR>

# C-z         easy C-x C-p {{{3

# Inspiration:
# https://www.reddit.com/r/vim/comments/78h4pr/plugins_andor_keybindings_you_couldnt_live_without/dou7z5n/
inoremap <unique> <C-Z> <ScriptCmd>custom.EasyCXCP()<CR><C-X><C-P>
#}}}1
# Options {{{1
# complete {{{2
...
#         endfor
#         lines->add( 'we_dont_want_this_' .. random_chars)
#     endfor
#
#     lines->setline(1)
#     :100 copy 100
...
if !executable('aiksaurus')
    ...
    system('sudo --stdin apt-get --yes install aiksaurus', pass .. "\n")
endif

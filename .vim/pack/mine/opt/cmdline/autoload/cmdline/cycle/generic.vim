...
#
#     noautocmd vimgrep //gj `find . -cmin -60 -type f` | cwindow
#                       ^                   ^
#                       tabstop 1           tabstop 2
#
...
# Interface {{{1
export def Set(arg_seq: string, ...cmds: list<string>): string #{{{2
    var first_cmd: string = cmds[0]
    var pos: number = FindTabstop(first_cmd)
    seq_and_cmds->add([arg_seq, cmds])
    # We don't install the mapping from here, because we want `:verbose` to give
    # a more useful location.
    return 'nnoremap <unique> <C-G>' .. arg_seq
        # We need the  full autoload name for the function,  because the mapping
        # can be installed from any script; not necessarily this one.
        .. ' <ScriptCmd>cmdline#cycle#generic#SetSeq(' .. string(arg_seq) .. ')<CR>'
        .. ':' .. first_cmd->substitute('⌖', '', '')
        .. '<C-R>=!!setcmdpos(' .. pos .. ') ? "" : ""<CR>'
enddef

export def Move(is_fwd = true): string #{{{2
    ...
    augroup ResetCycleIndex
        autocmd!
        autocmd CmdlineLeave : cycles[seq][0] = 0
    augroup END

    execute 'cnoremap <Plug>(cycle-new-cmd) ' .. new_cmd .. '<C-R>=!!setcmdpos(' .. pos .. ') ? "" : ""<CR>'
...
# cmds = ['cmd1', 'cmd2']
    var positions: list<number> = arg_cmds
        ->copy()
        ->map((_, v: string): number => FindTabstop(v))
...
def FindTabstop(rhs: string): number #{{{2
    # Why not simply `return stridx(rhs, '⌖')`?{{{
    #
...
export def SetSeq(arg_seq: string): string #{{{2
    seq = arg_seq
    return ''
enddef

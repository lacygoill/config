...
function graph#cmd(action, line1, line2) abort "{{{1
    silent update
    let cmd = a:action->matchstr('-compile\s*\zs\S\+')
    if empty(cmd)
        let cmd = 'dot'
    endif
    ...
    if a:action =~# '-compile'
        ...
        let funcname = a:action->matchstr('-\zs\S\+')
...
function graph#cmd_complete(arglead, cmdline, _p) abort "{{{1
    ...
    return a:arglead[0] ==# '-' || empty(a:arglead) && a:cmdline !~# '\%(-compile\|-show\)\s\+\w*$'
        \ ?     options->join("\n")
        \ :     ['circo', 'dot', 'dot2text', 'fdp', 'neato', 'sfdp', 'twopi']->join("\n")
endfunction

function graph#omni_complete(findstart, base) abort "{{{1
    if a:findstart
        let line = getline('.')
        let pos = col('.') - 1
...
function s:compile(cmd, line1, line2) abort "{{{1
    if !executable(a:cmd)
        try
    ...
    silent call printf('(%s -T' .. s:FORMAT .. ' %s -o %s 2>&1) | tee %s',
        \ a:cmd,
        \ shellescape(file),
    ...
    if getfsize(logfile)
        execute 'cfile ' .. escape(logfile, ' \"!?''')
    endif
    call delete(logfile)
    return 1
endfunction

function graph#create_diagram() abort "{{{1
    ...
    " \                     '(' .. path->fnamemodify(':h')->substitute('\V' .. $MY_WIKI, '$MY_WIKI', '') .. '.pdf)',
    call setline(lnum, line)

    " open a split to write source code of diagram
    split | execute 'edit ' .. path_to_dot
endfunction

function graph#edit_diagram() abort "{{{1
    ...
    let path = path->matchstr('\[.\{-}\](\zs.\{-}\ze)')
    let fname = path->fnamemodify(':t:r') .. '.dot'
    let path = path->fnamemodify(':h') .. '/src/' .. fname
    ...
    split | execute 'edit ' .. path
    nnoremap <buffer><expr><nowait> q reg_recording() != '' ? 'q' : '<ScriptCmd>quit<CR>'
    autocmd BufWritePost <buffer> ++once Graph -compile
endfunction

function s:interactive() abort "{{{1
    " FIXME: Would it be possible to support other commands (neato, twopi, …)?
    ...
    silent call system('dot -Txlib ' .. expand('%:p:S') .. ' &')
endfunction

function s:output_file() abort "{{{1
    return expand('%:p:h:h') .. '/' .. expand('%:t:r') .. '.' .. s:FORMAT
endfunction

function s:show(cmd,line1,line2) abort "{{{1
    ...
    silent call system(s:VIEWER .. ' ' .. s:output_file()->shellescape() .. ' &')
endfunction

function graph#undo_ftplugin() abort "{{{1

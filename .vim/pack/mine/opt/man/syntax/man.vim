...
#
# `manBold`, `manItalic`, `manUnderline` might be  cleared when a color scheme
# is set.   That's because  a color scheme  can run  `:highlight clear`.  Note
# that the  `default` argument does  not help here;  `:highlight clear` resets
...
highlight default link manBold Bold
highlight default link manItalic Italic
...
# keep going only for a man buffer (not included by some other syntax plugin)
if !(&filetype == 'man'
        # or if sourced to handle some backspaced text
        || search('\(.\)\b\1\|_\b.', 'cn') == 0)

...
export def InitPager() # {{{2
    ...
    # Corner Case: The first line of the buffer might be broken:{{{
    #
    #     $ man 3 Net::DNS::ZoneFile
    #     :echo getline(1)
    #     Net::DNS::ZoneFile(3pmUser Contributed Perl DocumentatiNet::DNS::ZoneFile(3pm)
    #                       ^                         ^---------^                  ^   ^
    #
    # Notice how  the parentheses  are unbalanced  and how  "Documentation" is
    # unexpectedly truncated.   If we  don't do  anything, this  would prevent
    # `:edit` from reloading the man page, because its name would be wrong:
    #
    #     ✘
    #     net::dns::zonefile(3pmuser_contributed_perl_documentatinet::dns::zonefile(3pm)
    #
    #     ✔
    #     net::dns::zonefile(3pm)
    #}}}
    if ref->count('(') != ref->count(')')
        ref = ref->matchstr('^[^()]\+') .. ref->matchstr('.*\zs([^)]\+)')
    endif
...
export def ExCmd( # {{{2
    ...
    try
        ...
        try
            # To suppress:
            #
            #     Error detected while processing function man#ExCmd:
...
def GetPath(sect: string, name: string): string # {{{3
    ...
    # search command. Previously, this function  would simply select the first
    # one.
    #
    # However, some  searches will report  matches that are incorrect:  man -w
    # strlen may return string.3 followed by strlen.3, and therefore selecting
    # the first  would get us  the wrong page.  Thus,  we must find  the first
    # matching one.
    #
    # There's yet another special case  here.  Consider the following:  If you
    # run man -w  strlen and string.3 comes  up first, this is  a problem.  We
    # should search for a matching named one in the results list.  However, if
    # you search for man -w clock_gettime, you will *only* get clock_getres.2,
    # which is the  right page.  Searching the results  for clock_gettime will
    # no longer work.  In this case, we should just use the first one that was
...
def GetPage(path: string): string # {{{3
    ...
    var cmd: list<string> =<< trim eval END
        ...
        man
    END
    return Job_start(cmd + ['--local-file', path])
enddef
...
def Job_start(cmd: list<string>): string # {{{3
    ...
    if cb_opts.exit_status != 0
        ...
        #
        #     man.vim: job error (PID 1826) man --where unknown: No manual entry for unknown
        #
    ...
    #
    # Edit: The issue comes from the fact that we return too early.
    ...
    #
    #     ['man', '-w', 'man']
    #     ['env', 'MANPAGER=cat', 'MANWIDTH=80', 'MAN_KEEP_FORMATTING=1', 'man', '--local-file', '/usr/share/man/man1/man.1']
    #     ['man', '-w', '-S', '1', 'ffmpeg-all']
    #     ['man', '-w']
    #     ['man', '-w', '-S', '1', 'ffmpeg-all']
    #     ['man', '-w', '/usr/share/man/man1/ffmpeg-all.1.gz']
    #     ['env', 'MANPAGER=cat', 'MANWIDTH=80', 'MAN_KEEP_FORMATTING=1', 'man', '--local-file', '/usr/share/man/man1/ffmpeg-all.1.gz']
    #
    # In particular, `man --where` seems useless...
...
def HighlightWindow() # {{{3
    ...
    if b:man_highlight.seen[lnum1 - 1 : lnum2 - 1]->index(false) == -1
        ...
        if b:man_highlight.seen->index(false) == -1
            # Warning: Do *not* remove all autocmds.
            # Only the ones associated to the current buffer.{{{
            #
            #     ✘
            #     autocmd! HighlightManPage
            #
            # We might  still have another  man buffer containing  lines which
            # our cursor hasn't visited yet.  If  we visit them later, we want
            # them to be highlighted.
            #}}}
            autocmd! HighlightManPage * <buffer>
...
def HighlightLine(line: string, linenr: number) # {{{3
    ...
    # TODO: Why did the plugin pass `line` to `Gmatch()`?{{{
    #
...
export def HighlightOnCursormoved() # {{{3
# We need to export this function for our `StdinReadPost` autocmd.
...
def HighlightTearDown() # {{{3
    ...
    if getbufinfo()
            ->indexof((_, info: dict<any>): bool => info.bufnr->getbufvar('&filetype') == 'man') == -1
        autocmd! HighlightManPage
        augroup! HighlightManPage
    endif
enddef

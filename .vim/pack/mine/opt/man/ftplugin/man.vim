...
import autoload '../autoload/link.vim'
...
#     &l:undolevels = ul
#
# Edit: The code  is commented because it  gives an error for  `:Man` (not for
# `$ man`).   When `:Man`  is used,  `'modifiable'`  is reset.   But not  when
# `$ man` is used.  What a mess.  Make sure this option is set consistently.

...
# TODO: Maybe we  should consider removing  the concept of a  default session.
# We never use it, and it adds some complexity to the plugin.

# TODO: Maybe add a command opening a  buffer showing all session names with a
# short  description.  When  you would  select one,  you would  have a  longer
# description in a popup window.

# TODO: When Vim starts, we could tell  the plugin to look for a `session.vim`
# file inside the working  directory, and source it if it  finds one, then use
# it to track  the session.  This would allow  us to not have to  name all our
# sessions.  Also,  `:STrack ∅` should save  & track the current  session in
# `:pwd`/session.vim.
#
# Edit: Wait.  How would we pause the tracking  of a session then?  I guess it
# would need to check  whether a session is being tracked  (easy), or has been
# tracked in the past (tricky?)...
...
augroup MySession
    ...
    #       pops into existence
    #
    #    2. update the session file frequently, and as long as `g:my_session`
    #       exists (IOW, track the session)
    #}}}
    #                     if something goes wrong, the function returns the string:
    ...
    #       ^---------^
    # We don't want the session to be saved only when we quit Vim, because Vim
    # could exit abnormally.
    #
    # Contrary to `BufEnter`, `BufWinEnter` is NOT fired for `:split` (without
    # arguments), nor for  `:split file`, `file` being already  displayed in a
    # window.
    #
    # But most  of the  time, Vim  won't quit abnormally,  and the  last saved
    # state of our session will be performed when `VimLeavePre` is fired.  So,
    # `VimLeavePre` will have the final say most of the time.

    autocmd TabClosed * timer_start(0, (_) => execute('execute Track()'))
    # We also save whenever we close a tabpage, because we don't want a closed
    # tabpage to be restored while we switch back and forth between 2 sessions
    # with `:SLoad`.  But, we can't  save the session immediately, because for
    # some reason, Vim would only save the last tabpage (or the current one?).
    # So, we delay the saving.

    # Why inspecting `v:servername`?{{{
    #
    # To not overwrite  the `session/last` file if we're editing  it outside a
    # session.  We sometimes need to do this to debug some issue.
    #}}}
    ...
    autocmd VimLeavePre * {
        ...
        if get(g:, 'MY_LAST_SESSION', '') != '' && v:servername != ''
            writefile([g:MY_LAST_SESSION], SESSION_DIR .. '/last')
        endif
    }
    autocmd User MyFlags {
        g:StatusLineFlag('global', '%{SessionStatus()}', 5, expand('<sfile>:p') .. ':' .. expand('<sflnum>'))
    }
augroup END
...
#
# How does it work?
# When an error might occur, we capture it and convert it into a string:
#
#     try
#         ...
#     catch
...
# Basically, the try  conditional + `execute function()` is  a mechanism which
# lets us  choose the context in  which an error might  occur.  Note, however,
# that in this case, it prevents  our `:WTF` command from capturing the error,
# because it will happen outside of a function.
#}}}
...
command -bar -bang -nargs=? -complete=custom,SuggestSessions SDelete Delete(<bang>0, <q-args>)
command -bar -nargs=1 -complete=custom,SuggestSessions SRename Rename(<q-args>)

# Warning: Do *not* give `-bar` to `:SLoad`.{{{
#
# It would  break `:SLoad #`  when executed  in Vim9  context (i.e.  from this
# script).  Vim  would parse `#` as  a comment leader, remove  it, and execute
# `:SLoad`.
#}}}
command -nargs=? -complete=custom,SuggestSessions SLoad Load(<q-args>)
...
def Delete(arg_bang: bool, session: string) #{{{2
    if !arg_bang
        Error('Add a bang')
        return
    endif
    ...
    elseif session_to_delete == get(g:, 'my_session', '')
        if exists('g:MY_PENULTIMATE_SESSION')
            ...
            # session file,  because `:SLoad #` will  take a last  snapshot of
            # the current session, which would restore the deleted file.
            SLoad #
    ...
    # Delete the session file, and if something goes wrong report what happened.
...
def Load(arg_session_file: string) #{{{2
    ...
    # During the sourcing, other issues might occur. {{{
    #
    # Every  custom function  that  we  invoke in  any  autocmd (vimrc,  other
    # plugin) might interfere with the restoration process.
    #
    ...
    #
    #     noautocmd source ~/.local/share/vim/session/default.vim
    ...
    #        a one-shot autocmd
    #
    # Solution: In  an  autocmd which  might  interfere  with the  restoration
    # process, test whether `g:SessionLoad` exists.  This variable only exists
    # while a session is being restored:
    #
    ...
    #     win_gotoid(orig)
    #
    # Edit:  I've commented the code because it interferes with `vim-cwd`.
    #
    # ---
    #
...
def LoadSessionOnVimenter() #{{{2
    ...
    # Check out our zshrc:
    #
    #     VIMSERVER=yes vim --servername "$server" "$@"
    #     ^-----------^
    #}}}
    ...
    var file: string = SESSION_DIR .. '/last'
    if filereadable(file)
        g:MY_LAST_SESSION = file
            ->readfile('', 1)
            ->get(0, '')
    endif
...
def SafeToLoadSession(): bool #{{{2
    ...
    # It's safe to automatically load a session during Vim's startup if, and only if:
    #
    #    - Vim is started with no files to edit.  If there are files to edit
    #      we don't want their buffers to be immediately lost by a restored
    #      session.
    #
    ...
    #      Otherwise, loading it in a 2nd instance would give `E325`.
enddef
...
def SaveOptions(): dict<any> #{{{2
    ...
    # options while restoring a session.  At the end of the process, Vim wants
    # to set those options to values expected by the user.  The only values it
    # knows the user  might expect, are the  ones which were used  at the time
    # the session file  was created.  Therefore at the end  of a session file,
    # Vim writes `set winheight={current value}`:
    #
    ...
    #
    # But  this implies  that when  you load  a session,  these options  might
    # not  be  preserved;  in  particular,  when  you  restart  Vim,  and  the
    # latter  automatically sources  the  last session,  in  effect, Vim  will
    # have  remembered the  values of  these  options from  the last  session.
    ...
    #
    # Edit: This *might* have been fixed by this patch:
    # https://github.com/vim/vim/releases/tag/v8.2.4980
    #}}}
    return {
        shortmess: &shortmess,
...
def SessionLoadedInOtherInstance(session_file: string): list<any> #{{{2
    ...
    var it_is_not_in_this_session: bool = buffers
        ->indexof((_, buf: string): bool => buflisted(buf)) == -1
...
def SessionDelete() #{{{2
    ...
    #
    #     ~/.local/share/vim/session/default.vim
    v:this_session = ''
enddef
...
def Track(on_vimleavepre = false): string #{{{2
    # This function saves the current session, if, and only if, `g:my_session`
    # exists.  In the session file, it adds the line:
    #
    #     g:my_session = v:this_session
    #
    ...
    # update the session if, and only if, (or as soon as) this variable exists
    if exists('g:my_session')
        try
            if on_vimleavepre
                # Why do it?{{{
            ...
            # might not work  all the time.  Sometimes they  give `E788`.  For
            # us,  it happens  when we  open the  quickfix window  (`:copen`).
            # MRE:
            #
            ...
            # buffer when `'filetype'` is set to `qf`, which is now forbidden.
            ...
            #     Vim(mksession):E11: Invalid in command-line window; <CR> executes, CTRL-C quits: mksession! /home/lgc/.local/share/vim/session/C.vim
            #}}}
        catch
...
def TweakSessionFile(file: string) #{{{2
    ...
    insert(body, 'g:my_session = v:this_session', -3)
    # Why twice?{{{
    #
...
def VimQuitAndRestart() #{{{2
    ...
    # For some reason, `kill -USR1` kills  the shell parent, unless we started
    # vim with `nv`.   Let's make sure we did, otherwise,  the tmux pane might
    # be closed, which is confusing.
    if $VIMSERVER == ''
        echo 'vim was not started with nv'
        return
    endif
    ...
    #}}}
    var shell_parent_pid: string = '$(ps --pid=' .. getpid() .. ' --format=ppid=)'
...
def WhereDoWeSave(): string #{{{2
    ...
    elseif isdirectory(sfile)
        ...
        #     it should be in ~/.local/share/vim/session/par.vim˜
        #
...
nnoremap <unique> <Space>R <ScriptCmd>VimQuitAndRestart()<CR>
...
const SESSION_DIR: string = $'{$HOME}/.local/share/vim/session'
...
# Invoke `:mksession` on:
#
#    - `~/.local/share/vim/session/file`
#    - `/path/to/file`
#    - `cwd/path/to/file`
#
# ... if, and only if, `file` doesn't exist.
#
...
# Rename current session into `~/.local/share/vim/session/foo`.
#
# ---
#
#     :SLoad
...
#     :SDelete! foo
#
# Load / Delete session `foo` stored in `~/.local/share/vim/session/foo.vim`.
#
# ---
#
...
# I've added it  because `:SLoad dir/` create a session file  in `dir/`, which
# is not `~/.local/share/vim/session`.
#

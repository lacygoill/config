...
export def Split(in_newtab = false) #{{{2
...
export def FoldLevel() #{{{2
    &l:foldlevel = &foldclose == 'all' ? 0 : 99
enddef

export def FoldText(): string #{{{2
    var pat: string = '\(.*─\s\)\(.*\)/'
...
export def Open(arg_dir: string, nosplit: bool) #{{{2
    if !executable('tree')
    ...
    # save current filename to position the cursor on it
    ...
    var tmp_file: string = tempname()
        ...
        # Besides it  would give an  error when `SaveBuffer()` would  be invoked
        # (`:update` would fail; E502).
        .. (dir == '/' ? '' : dir)
    if nosplit
        execute 'edit ' .. tmp_file
    else
        execute 'topleft :' .. get(t:, 'fex_winwidth', &columns / 3) .. ' vnew ' .. tmp_file
    endif
enddef
var current_file_pos: string

export def Populate(path: string) #{{{2
    ...
    if current_file_pos != ''
        autocmd BufWinEnter <buffer> ++once {
            search(current_file_pos)
            current_file_pos = ''
        }
    endif
enddef

export def Preview() #{{{2
...
export def RelativeDir(who: string) #{{{2
    var curdir: string = Getcurdir()
...
def GetIgnorePat(): string #{{{2
    ...
    var ignore_pat: string = &wildignore
        ...
        # We may get empty matches, or something like `*.*` because of (in vimrc):{{{
        #
        #     &wildignore ..= ',' .. &undodir .. '/*.*'
        #
...
def GetTreeCmd(dir: string): string #{{{2
    var short_options: string = '-'
        ...
        .. 'n'
        .. (hide_dot_entries ? '' : ' -a')
        # Sort by  version which  feels more  natural to  people, when  the text
...
def UseCache(dir: string) #{{{2
    ...
    if cache[dir]->has_key('pos')
        ...
        # Vim will  re-position the  cursor on the  first line  afterward (after
        # BufEnter).
        #}}}

...
export def FormatEntries() #{{{1
    var pat: string = glob2regpat(&wildignore)
        ->substitute(',', '\\|', 'g')
        # An absolute path must match from the start.
        # Warning: Don't move this substitution before the next one.{{{
        #
        # In the end, both substitutions would replace something like `\|tags` with:
        #
        #     \|^/tags
        #       ^
        #       ✘
        #}}}
        ->substitute('\\|\zs\ze/', '^', 'g')
        # A file name must match as a whole; not just at the end.{{{
        #
        #     \|tags
        #     →
        #     \|/tags
        ...
        #       ✔
        #
        # For    example,     this    is    useful    to     avoid    ignoring
        # `~/.config/ctags/python.ctags`   even  if   `'wildignore'`  contains
        # `tags`.
        #}}}
        ->substitute('\\|\zs\ze\w', '/', 'g')
    pat = '\%(' .. pat .. '\)$'
...
def GetMetadata(line: string, with_filename = false): string #{{{1
    ...
    if ftype == 'dir'
        ...
        #
        #     silent var human_fsize: string = system('du -sh ' .. shellescape(file))
        #         ->trim("\n", 2)
        #         ->matchstr('\S\+')
        #
...
export def Preview() #{{{1
    var file: string = getline('.')
    if filereadable(file)
        execute 'pedit ' .. file
        var winid: number = lg.Win_getid('P')
...
export def PrintMetadata(auto = false) #{{{1
    ...
    if auto
        if !exists('#FexPrintMetadata')
            ...
            augroup FexPrintMetadataAndPersist
                autocmd!
    ...
    PrintMetadataImpl(in_visualmode)
enddef

def PrintMetadataImpl(in_visualmode: bool) #{{{1
    var lines: list<string> = in_visualmode ? getline("'<", "'>") : [getline('.')]
    lines->filter((_, v: string): bool => v !~ '^#')
    if lines->empty()
        return
    endif
...
def AutoMetadata() #{{{1
    augroup FexPrintMetadata
        ...
        autocmd CursorMoved <buffer> {
            if get(b:, 'fex_last_line', 0) != line('.')
                b:fex_last_line = line('.')
                PrintMetadataImpl(false)
            endif
        }
    augroup END
enddef

export def ToggleDotEntries() #{{{1
    hide_dot_entries = !hide_dot_entries
    Dirvish %
enddef

export def TrashPut() #{{{1
    silent system('trash-put ' .. getline('.')->shellescape())
    edit
enddef

export def DirvishUp() #{{{1
    ...
    if file != '' && !isdirectory(dir)
        ...
        #
        # This happens for example in `:Tree`.
        #
        # MRE:
        #
        ...
        #
        #     # ~/.vim/pack/vendor/opt/vim-dirvish/autoload/dirvish.vim:28

...
def GetRawTrace(max_dist = 3): list<dict<any>> #{{{2
    ...
    while i >= 0
        ...
        # We ignore error messages given from pseudo-files under `/proc/`.{{{
        #
        ...
        # the code which given the errors.
        #
        ...
        if msgs[i] =~ '^Error detected while \%(processing\|compiling\)'
            ...
            errors->add({
                stack: stack
                     ->split('\.\.')
                     # Might be necessary when the error is given from a script sourced manually:{{{
                     #
                     ...
                     #                                                                                 ^-------^
                     #
                     # When  an  error  is  given  from  a  function  which  was
    ...
    # reverse the errors, because I like reading them in their chronological order
...
def BuildQfl(errors: list<dict<any>>): list<dict<any>> #{{{2
    ...
    for err: dict<any> in errors
        # to get a more accurate location for an error in a lambda executing a block
        var last_lnum: number = 0
        ...
        #     'FuncB[34]'
        #}}}
        for [i: number, call: string] in err.stack->items()
            ...
            # If the name of  a function contains a slash, or a  dot, it's not a
            # function, it's  a file.  It happens  when the error occurred  in a
            # sourced file,  like a ftplugin (put  an invalid command in  one of
            # them to reproduce).
            if name =~ '[/.]'
                qfl->add({
                    text: '',
                    filename: name,
                    lnum: call->GetLnumBody(),
                    type: err.type
                })
            ...
            #       '   enddef']
            #}}}
            var def: list<string> = GetFunctionDefinition(name)

            # If we failed  to retrieve a definition, bail out,  and process the
            # next frame in the stack.
            if !def->IsValid()
                # If the  frame is for  a lambda executing a  block, remembering
                # the address of the line in its body might help us later.
                # Note that  a lambda can  execute a  block either via  a nested
                # `:def` function or `:help inline-function`.
                # Also, note that  a lambda is automatically removed  as soon as
                # it's no longer referenced anywhere.
                if def->LambdaType() == 'block'
                    last_lnum += call->GetLnumBody()
                endif
                continue
            endif
            ...
            var lnum: number = def[1]
                ->matchstr('Last set from .\+ line \zs\d\+')
                ->str2nr()
            # An error in a lambda is a special case.{{{
            #
            # There are 2 types of lambdas.
            # The ones which evaluate an expression (most frequent).
            # The ones which execute a block (`:help inline-function`).
            #
            # For the first type, the given line number is misleading:
            #
            #     vim9script
            #     def Func()
            #         [[]]->map((_, v) =>
            ...
            #
            # Notice that the line number is  1, even though the error is on
            # line 3  in the  lambda.  But  1 is  always given,  because Vim
            # joins all the lines in a lambda's expression:
            #
            #     :def <lambda>1
            #        def <lambda>1(_: any, v: any): list<any>
            #     1  return []         +         [[][0]]         +         []
            #        enddef
            #
            # We should ignore this 1, otherwise we might end up on a weird line:
            #
            #     vim9script
            #     def Func()
            #         [[]]->map((_, v) => [] + [[][0]] + [])
            #     enddef # we don't want to end up here
            #     Func()
            #}}}
            if name !~ '^<lambda>\d\+' || def->LambdaType() == 'block'
                lnum += call->GetLnumBody()
            endif
            # The previous frame  might have been discarded because  of a lambda
            # which is no longer referenced.  The current frame will only get us
            # the header  of the outer function.   Fortunately, we've remembered
            # the address of the error relative to this header.  Let's add it.
            if last_lnum != 0
                lnum += last_lnum
                last_lnum = 0
            endif
...
def SetQfl(qfl: list<dict<any>>) #{{{2
    setqflist([], ' ', {
        ...
        context: {
            origin: 'mine',
            matches: [['Conceal', 'barbar']],
        }
    })
    # no need to make Vim open the quickfix window if it's already open
    if &filetype != 'qf'
...
#}}}1
# Utilities {{{1
def GetLnumBody(call: string): number #{{{2
    return call
        ->matchstr('\[\zs\d\+\ze\]$')
        ->str2nr()
enddef
...
def LambdaType(def: list<string>): string #{{{2
# We use the size of the body to  infer which type of lambda we're dealing with.
# In theory, that's wrong, but in practice, it should be good enough.
# We will rarely encounter  a lambda executing a block with  only a single line.
# And even  if that happens, we  will still end up  on its header; the  error is
# right below; that's not hard to find.
    if def->copy()
            ->filter((_, line: string): bool => line =~ '^\s*\d')
            ->len() == 1
            && def[2] =~ '^\s*\d\+\s*return\>'
        return 'expr'
    endif
...
def IsValid(def: list<string>): bool #{{{2
    return def->len() >= 2
enddef

...
#
# `U+00A0` is  really rendered  as an  *invisible* glyph; not  ` `. If  it was
# rendered as  ` `, when  you're in  a submode entered  from insert  mode, the
# character after  the cursor would be  hidden until the timeout.   That's not
# the case; it always remains visible.
...
export def Enter( #{{{2
    ...
    #
    #     <Plug>(sm-show:scrollwin) <ScriptCmd>ShowSubmode('scrollwin')<CR>
    #
    #     <Plug>(sm-prefix:scrollwin)_____  <ScriptCmd>OnLeavingSubmode()<CR>
    #}}}
...
def GetMapCmd( #{{{2
    ...
    #     inoremap <Plug>(sm-show:scrollwin) <ScriptCmd>ShowSubmode('scrollwin')<CR>
    execute printf('%snoremap %s <ScriptCmd>ShowSubmode(%s)<CR>', mode, plug_show, string(name))

    #     inoremap <Plug>(sm-prefix:scrollwin) <ScriptCmd>OnLeavingSubmode()<CR>
    execute printf('%snoremap %s <ScriptCmd>OnLeavingSubmode()<CR>', mode, plug_prefix)
    ...
    return mode .. 'map '
        ...
        #     →     ShowSubmode('scrollwin')
        .. plug_show
        #     <Plug>(sm-prefix:scrollwin)_____
        #     →     <ScriptCmd>OnLeavingSubmode()<CR>
        .. plug_prefix
enddef

def BeforeExe() #{{{2
    # save the count immediately, in case we want to refer to it in our mapping
    g:submode_count = v:count1
...
#}}}
    ...
    if when == 'now'
        var suffix: string
        if msg_suffix[submode] != null_function
    ...
    # TODO: What follows is irrelevant with regard to the function name.

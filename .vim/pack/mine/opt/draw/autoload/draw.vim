...
# source  it,  then the  next  time  we will  toggle  the  state of  the  plugin
# with  `m_`, `m<Space>`,  `ChangeState()`  will save  the  drawing mappings  in
...
# Interface {{{1
export def BoxPrettify(line1: number, line2: number) #{{{2
...
export def ChangeState(erasing_mode: bool) #{{{2
    if state == 'disabled'
        ...
        original_mappings_visual = mapping.Save([
            'j',
            'k',
            'ma',
            'mb',
...
export def Stop() #{{{2
    ...
    if original_mappings_normal != []
        mapping.Restore(original_mappings_normal)
    endif
    if original_mappings_visual != []
        mapping.Restore(original_mappings_visual)
    endif
...
def AboveFirstLine(key: string): bool #{{{2
    return ['<Up>', '<PageUp>', '<Home>', '^']->index(key) >= 0
        && state == 'drawing' && line('.') == 1
enddef

def BeyondLastLine(key: string): bool #{{{2
    return ['<Down>', '<PageDown>', '<End>', 'v']->index(key) >= 0
...
def ArrowCycle(is_fwd: bool) #{{{2
    ...
    var x0: number = [virtcol("'<", true)[0], virtcol("'>", true)[0]]->min()
    var x1: number = [virtcol("'<", true)[0], virtcol("'>", true)[0]]->max()
    ...
    var new_state: string = states[
        (states->index(cur_state) + (is_fwd ? 1 : -1)) % len(states)
    ]
...
def GetCharsAround(where: string): string #{{{2
    ...
    return where == 'before'
        ...
        ?     getline('.')[charcol]
        : where == 'above'
        ?     (line('.') - 1)->getline()->matchstr('\%' .. virtcol('.', true)[0] .. 'v.')
        # character below
        :     (line('.') + 1)->getline()->matchstr('\%' .. virtcol('.', true)[0] .. 'v.')
enddef
...
def MappingsInstall() #{{{2
    ...
    for key: string in [
        ...
        execute printf('nnoremap %s %s <ScriptCmd>Draw(%s)<CR>', args, key, string('<lt>' .. key[1 :]))
    endfor
    ...
    for key: string in [
        ...
        execute printf('nnoremap %s %s <ScriptCmd>Draw(%s)<CR>', args, key, string(key))
    endfor
    ...
    for key: string in [
        ...
        execute printf('nnoremap %s %s <ScriptCmd>UnboundedVerticalMotion(%s)<CR>',
    ...
    xnoremap <nowait> ma <C-\><C-N><ScriptCmd>Arrow()<CR>
    xnoremap <nowait> mb <C-\><C-N><ScriptCmd>Box()<CR>
    xnoremap <nowait> me <C-\><C-N><ScriptCmd>Ellipse()<CR>
    xnoremap <nowait> mm <C-\><C-N><ScriptCmd>ArrowCycle(true)<CR>
    xnoremap <nowait> mM <C-\><C-N><ScriptCmd>ArrowCycle(false)<CR>

    nnoremap <nowait> m? <ScriptCmd>Stop() <Bar> help my-draw-it<CR>
enddef

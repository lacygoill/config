...
import autoload '../autoload/search.vim'

# TODO: Prevent the plugin from highlighting matches after a search run from operator-pending/visual mode.{{{
#
# You can't  do that right now,  because `mode()` returns `v`,  `V`, `^V` when
# the search command-line  has been entered from visual mode,  and `c` when it
# was entered from operator-pending mode.
#
# You need to wait for `mode(true)` to  be able to return `c/v` and `c/o` (see
# `:help todo /c\/o`).  More generally, disable anything fancy when the search
# command-line was not entered from normal mode.
#}}}
...
nnoremap <Plug>(ms-N)     N
nnoremap <Plug>(ms-prev) <ScriptCmd>search.RestoreCursorPosition()<CR>
...
nmap <expr><unique> gd search.WrapGd(true)
nmap <expr><unique> gD search.WrapGd(false)

nmap <expr><unique> n search.WrapN(true)
nmap <expr><unique> N search.WrapN(false)
...
# Customizations (nohls, index, ...) {{{2

nnoremap <Plug>(ms-restore-registers) <ScriptCmd>search.RestoreRegisters()<CR>
nnoremap <expr> <Plug>(ms-view) search.View()
nnoremap <Plug>(ms-nohls) <ScriptCmd>search.NoHls()<CR>
...
#}}}
nnoremap <Plug>(ms-index) <ScriptCmd>search.Index()<CR>
...
#                             │               │
nmap <Plug>(ms-custom) <Plug>(ms-nohls)<Plug>(ms-view)<Plug>(ms-index)
#                                                            │
#                            print `[12/34]` kind of message ┘

# We need this mapping for when we leave the search command-line from visual mode.
xnoremap <Plug>(ms-custom) <ScriptCmd>search.NoHls()<CR>
...
# disable it.
#}}}
inoremap <Plug>(ms-nohls) <ScriptCmd>search.NoHlsOnLeave()<CR>
...
augroup HoistNoic
    ...
    #}}}
    autocmd User MyFlags silent! g:StatusLineFlag('global', '%2*%{!&ignorecase && state("mox") == "" ? "[noic]" : ""}', 17,

...
if !exists('g:awk_indent_stat_continue')
  g:awk_indent_stat_continue = 1.5
endif
...
def ContinueLineIndent(alnum: number, cline: string): list<any>
  ...
  elseif line =~ '\]' && PairBalance(line, '\]', '\[') > 0
      && IsTailContinue(line) && IsTailContinue(pline)
    ind = NestContinueIndent(line, lnum, cline, '\[', '\]')
  elseif line =~# '^\s*for\s*(.*;\s*$' && UnclosedPair(line, '(', ')')
  ...
  elseif line =~ '[^<>=!]==\@!\s*[^\\[:blank:]]' && IsTailContinue(line) && !IsTailContinue(pline)
    ind = GetMatchWidth(line, lnum, '[^<>=!]=\s*\zs\S')
  elseif line =~ '^\s\+\h\w*\s\+[^-+/*%^=\\[:blank:]]'
      && IsTailContinue(line) && !IsTailContinue(pline)
    ind = GetMatchWidth(line, lnum, '\h\w*\s\+\zs\S')
  elseif IsTailContinue(line, true) && !IsTailContinue(pline)
    ind += ind > 0 ? shiftwidth()
        : float2nr(shiftwidth() * (shiftwidth() > 5 ? 0.5 : (shiftwidth() < 4 ? 2 : 1.5)))
  endif
...
def MorePrevLineIndent(pline: string, pnum: number,
  ...
  if IsTailContinue(line) || !!PairBalance(line, '(', ')')
    return aind
  endif
...
def JoinContinueLine(lnum: number, ...line: list<string>): list<any>
  ...
  var balance = PairBalance(rlist[0], '(', ')')
  pn = rlist[1]
  while PrevNonBlank(pn)
    ...
    const pp_balance = PairBalance(pline, '(', ')')
    if balance < 0
      balance += pp_balance
...
def CleanPair(aline: string, i1: string, i2: string): string
  ...
  while last != line
    last = line
    line = substitute(line, i1 .. '[^' .. i1 .. i2 .. ']*' .. i2, '\=repeat("x", strlen(submatch(0)))', 'g')
  endwhile
  return line
enddef
...
def HideStrComment(aline: string): string
  ...
  var line = substitute(aline, '\\\@1<!\%(\\\\\)*\\.', '\=repeat("x", strlen(submatch(0)))', 'g')
  line = substitute(line, '\C"[^"]*"\|\%([^])._a-zA-Z0-9[:blank:]]\|^\)\s*\zs'
        .. '/\%(\%(\[\^\]\|\[\]\|\[\)\%(\[\([:=.]\)[^]:=.]\+\1\]\|[^]]\)*\]\|[^/[]\)*/\|'
        .. '/\%(\%(\[\^\]\|\[\]\|\[\)\%(\[\([:=.]\)[^]:=.]\+\1\]\|[^]]\)*\]\|[^/[]\)*/'
        .. '\ze\s*\%([^(\\._a-zA-Z0-9[:blank:]]\|$\)', '\=repeat("x", strlen(submatch(0)))', 'g')

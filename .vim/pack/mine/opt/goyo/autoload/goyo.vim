...
# Interface {{{1
export def Execute(bang: bool, dim: string) #{{{2
# `dim` = dimensions
...
export def Start(without_hl = true) #{{{2
...
export def Enter() #{{{2
    ...
    if without_highlighting
        ...
        # Some text  might be highlighted  via a  text property rather  than a
        # syntax plugin (e.g. `end` keyword in `fish` file).  Clear it too.
        prop_clear(1, line('$'))
    endif
enddef
...
def HideStatusline() #{{{2
    # TODO: Not sure how, but once,  the statusline disappeared after quitting
    # goyo mode.  And it seems that the local value of `'statusline'` is never
    # restored...
    &l:statusline = ' '
enddef
...
def MapsResize(): list<string> #{{{2
    var commands: dict<string> = {
        '=': '<ScriptCmd>t:goyo_dim = ParseArg(t:goyo_dim_expr) <Bar> ResizePads()<CR>',
        '>': '<ScriptCmd>t:goyo_dim.width = winwidth(0) + 2 * v:count1 <Bar> ResizePads()<CR>',
        '<': '<ScriptCmd>t:goyo_dim.width = winwidth(0) - 2 * v:count1 <Bar> ResizePads()<CR>',
        '+': '<ScriptCmd>t:goyo_dim.height += 2 * v:count1 <Bar> ResizePads()<CR>',
        '-': '<ScriptCmd>t:goyo_dim.height -= 2 * v:count1 <Bar> ResizePads()<CR>'
    }
...
nnoremap <Plug>(goyo-resize) <ScriptCmd>ResizePads()<CR>

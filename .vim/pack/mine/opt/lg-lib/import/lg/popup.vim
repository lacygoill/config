...
#
# Switch `DEBUG` to `true` in this file.
...
#     const LOGFILE: dict<string> = {popup-vim: '/tmp/...', other-feature: '/tmp/...'}
#
#     ...
#                                   new argument
#                                   v-----v
...
const LOGFILE: string = $'{$TMPDIR}/vim/popup-window-log.vim'
if DEBUG
    ['vim9script']->writefile(LOGFILE)
endif
const MAX_ZINDEX: number = 32'000

# Interface {{{1
export def Create(what: any, opts: dict<any>): dict<number> #{{{2
...
export def Notification( #{{{2
    ...
    var opts: dict<any> = arg_opts->extend(n_opts, 'keep')
    return Create(lines, opts)
enddef
#}}}1
# Core {{{1
...
def Terminal(what: any, opts: dict<any>): dict<number> #{{{2
    ...
    if IsTerminalBuffer(what)
        ...
        var cmd: string = 'var bufnr = term_start($INTERACTIVE_SHELL ?? &shell, {'
            .. "hidden: true, "
            .. "term_finish: 'close', "
            .. "term_kill: 'hup'"
            .. '})'
        Log(cmd, funcname, expand('<slnum>')->str2nr())
        bufnr = term_start($INTERACTIVE_SHELL ?? &shell, {
            hidden: true,
            term_finish: 'close',
            term_kill: 'hup'
        })
...
def FireTerminalEvents() #{{{2
    ...
    if exists('#ModeChanged#nt:t') | doautocmd <nomodeline> ModeChanged nt:t | endif
enddef

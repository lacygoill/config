...
# TODO: Remove this  block once  you find  a fix for  this issue  (which affects
# xterm but not kitty):
...
# Warning: Do *not* enable modifyOtherKeys when Vim runs in tmux.{{{
#
# Tmux only supports modifyOtherKeys=1, and Vim only supports modifyOtherKeys=2.
#
# This can cause bugs.  For example:
#
#     vim -Nu NONE --cmd 'let [&t_TI, &t_TE] = ["\e[>4;2m", "\e[>4;m"]' +'nnoremap <C-L> <C-W><C-L>' +vsplit +vsplit
#     # press C-L: the focus switches to the window on the right (expected)
#     # press C-;
#     # press C-L: nothing happens (not expected)
#}}}
...
#}}}
# But not in a terminal where `modifyOtherKeys` is enabled, nor in kitty, nor in the GUI.
# No need to, everything works fine there.
const USE_FUNCTION_KEYS: bool = &t_TI !~ "\<Esc>\\[>4;[12]m"
    && &term !~ 'kitty'
    && !has('gui_running')
...
const MAP_CMDS: list<string> = getcompletion('*map', 'command')
    ->filter((_, v: string): bool => v =~ '^\l\=\%(nore\)\=map$')
...
if USE_FUNCTION_KEYS
    ...
    # The issue affects Vim if, and only if, one of these statements is true:
    #
    ...
    #     No mapping found
    #
    # Solution: Re-run the command after replacing `<m-t>` with `<F31>`.
    augroup FixMapMeta
        autocmd!
        autocmd CmdlineLeave : FixMapMeta()
    augroup END
    def FixMapMeta()
        var cmdline: string = getcmdline()->trim(": \t")
        var pat: string = '^\%(verb\%[ose]\s\+\)\=\(\S\+\)\s\+<[mM]-\(\a\)>$'
        if cmdline !~ pat
            return
        endif
        var matchlist: list<string> = cmdline->matchlist(pat)->slice(1, 3)
        if matchlist->len() < 2
            return
        endif
        var [mapcmd: string, letter: string] = matchlist
        mapcmd = mapcmd->fullcommand()
        if MAP_CMDS->index(mapcmd) == -1
            return
        endif
        var n: number = letter->char2nr() - 85
        var fixed_cmd: string = cmdline->substitute('<\zs[mM]-\a\ze>', 'F' .. n, '')
        RerunFixedCmd = () => feedkeys(':' .. fixed_cmd .. "\<CR>", 'in')
...
# Interface {{{1
export def Meta(mapping: string): string #{{{2
    try
        ...
        # Vim conflates `<M-X>` with `<M-S-X>`.{{{
        #
        # In conjunction with `<unique>`, this can lead to unexpected errors:
        #
        #     $ vim -Nu NONE -g +'nnoremap <unique> <M-G> <Scriptcmd>echo "M-G"<CR>' \
        #                       +'nnoremap <unique> <M-S-G> <Scriptcmd>echo "M-S-G"<CR>'
...
export def MetaChord(key: string, symbolic = false): string #{{{2
...
export def Save( #{{{2
    ...
    # several modes.
    # `Restore()` will restore the normal mode  mapping, but it will not restore
    # the mappings in the other modes  (i.e. it won't remove the mappings you've
    # installed in the other modes).
    #
    #     nnoremap <C-Q> <Esc>
    #     var save: list<dict<any>> = Save('<C-Q>', '')
    #     noremap <C-Q> <Esc><Esc>
    #     Restore(save)
    #     map <C-Q>
    #     n  <C-Q>       * <Esc>˜
    ...
    #     no <C-Q>       * <Esc>˜
    #
    #     var save: list<dict<any>> = Save('<C-Q>', 'n')
    #     Restore(save)
    #     map <C-Q>
    #     n  <C-Q>       * <Esc>˜
    #     o  <C-Q>       * <Esc>˜
    ...
    for key: string in keys
        ...
        #
        #     var save: list<dict<any>> = Save('<C-Q>', 'nxo')
        #                                                ^^^
        #}}}
...
# Usage:{{{
#
#     var my_global_mappings: list<dict<any>> = Save(['key1', 'key2', ...], 'n')
#     var my_local_mappings: list<dict<any>> = Save(['key1', 'key2', ...], 'n', true)
#}}}

export def Restore(save: list<dict<any>>) #{{{2
    # Why?{{{
    #
    ...
    for maparg: dict<any> in save
        ...
        #
        # And you need  to save and restore some properties  of the buffer where
        # you re-install  the mapping;  like “was  it unlisted?”,  “was it
        # unloaded?”.
        #
...
#     Restore(save)
#
# `save` is a list obtained earlier by calling `Save()`.
...
def ReInstall(maparg: dict<any>) #{{{2
    ...
    #         .. maparg.rhs
    #
    # It no longer seems reliable enough.
    # It  doesn't seem  to correctly  restore  an expression  mapping which  was
    # installed from a Vim9 script.
    # As a test, enable then disable  our custom “drawing” mode, then try to
    # move with `j` and `k`:
    #
    #     E117: Unknown function: JK
    #
    # ---
    #
    # I'm not sure how reliable `mapset()`  is for mappings installed in several
    # modes, via `:map!` for example.  But for the moment, it seems good enough.
    # See here for more info about this possible future issue:
    # https://github.com/vim/vim/issues/6118
    #}}}
    mapset(maparg)
enddef
#}}}1

...
export def Scroll(lhs: string): string #{{{2
    if util.HasPreview()
        ScrollPreview(lhs)
        return ''
    endif

    var popup: number = util.LatestPopup()
    if popup > 0
    ...
    if lhs == 'C-u'
        ...
        &operatorfunc = function('readline.ChangeCaseWord', [true])
        # Don't write `normal! g@l`.
        # It would not always work as expected for a (possible) later dot command.
        return 'g@l'
    endif
...
def ScrollPopup(lhs: string, winid: number) #{{{2
    ...
    # For some reason, this is necessary after scrolling way beyond the last line with the mouse.{{{
    #
    # I think that's because then, `firstline` is set to 1, which seems wrong.
    # From `:help popup_create-arguments`:
    #
    #    > First buffer line to display.
    #    > [...]
...
#}}}
    if ['j', 'k', 'gg', 'G', 'C-d', 'C-u']->index(lhs) < 0
        return false
    endif
    if !should_scroll_tmux_previous_pane
        var tmux_cmd: string =
             # to check that the previous pane is running a shell
             'tmux display -p -t "{last}" "#{pane_current_command}"'
             # to check that the previous pane is in the current window
            .. '\; display -p -t "{last}" "#{window_index}"'
            .. '\; display -p             "#{window_index}"'
             # to check that the current window is not zoomed
    ...
    if should_scroll_tmux_previous_pane
        _j = printf('tmux last-pane ; copy-mode ; send-keys -X %s ; last-pane', {
            k: 'scroll-up',

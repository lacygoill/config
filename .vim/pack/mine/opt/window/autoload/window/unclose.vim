...
export def Restore() #{{{2
    ...
    # Sometimes the command-line takes multiple  lines.  For example, that can
    # happen if you execute a C file (which is alone in its tabpage) with `||`
    # – opening a new tmux pane –,  then close its window, – closing the
    # tmux pane  –, then restore it.   To prevent that, let's  make sure the
    # window is maximized.
    if winnr('$') == 1
        wincmd _
    endif

...
# Interface {{{1
export def Main(type = '', alnum1 = 0, alnum2 = 0): string #{{{2
    if type == ''
        &operatorfunc = Main
        return 'g@'
    endif

    if &l:commentstring->empty()
        echo "'commentstring' is not set"
        return ''
    endif
    ...
    var lnum1: number
    var lnum2: number
    if alnum1 != 0 && alnum2 != 0
        [lnum1, lnum2] = [alnum1, alnum2]
    else
    ...
    for lnum: number in range(lnum1, lnum2)
        ...
        var l: string
        var r: string
        [l, r] = util.MaybeTrimCml(line, l_, r_)
...
def DoWeUncomment(
    ...
    for lnum: number in range(lnum1, lnum2)
        ...
        if line =~ '\S' && !util.IsCommented(line, l, r)
            return false
        endif
    endfor
    return true

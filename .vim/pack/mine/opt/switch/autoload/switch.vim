...
const SWITCHABLE_TOKENS: list<list<string>> = [
    ['==', '!='],
    ['=~', '!~'],
    # We could also support strict inequality operators like this:{{{
    #
    #     ['>', '<', '>=', '<='],
    #
    # But it could wrongly operate on an opening `<` or closing `>` in a Vim keysym.
    # In a Vim file (and possibly other filetypes too), we would need to inspect
    # the text under the cursor to prevent mistakes.  Not worth the trouble.
    #}}}
    ['>=', '<='],
    ['enable', 'disable'],
    ['on', 'off'],
...
var TOKENS_MAP: dict<dict<string>> = {increment: {}, decrement: {}}
{
    for l: list<string> in SWITCHABLE_TOKENS
        ...
        for [i: number, token: string] in l->items()
        ...
        for token: string in tokens
            ...
            for offset: number in len->range()
                if line->strpart(col - 1 - offset, len) == token
                    if token =~ '^\w\+$'
                        # don't replace `no` (opposite of `yes`) in `normal`
                        && line !~ '\<\%' .. (col - offset) .. 'c' .. token .. '\>'
                        continue
                    endif

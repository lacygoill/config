...
export def WeCanRefactor( #{{{2
    ...
    if search_results->index(0) >= 0
    || !call(ContainsPos, [lnum0, col0] + args)
    || ContainsEmptyOrCommentedLine(lnum1, lnum2)
        return FinishRef(_this .. ' not found')
    endif
    if !bang && call(Confirm, ['refactor into ' .. into_that] + args) != 'y'
        return FinishRef()
    endif
    return true
enddef
...
def ContainsPos( #{{{2
    ...
    # return  1 if,  and only  if, the  position `[lnum0, col0]`  is somewhere
    # inside the characterwise text starting  at `[lnum1, col1]` and ending at
    # `[lnum2, col2]`

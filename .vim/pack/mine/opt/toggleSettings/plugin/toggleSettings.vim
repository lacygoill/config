...
#                       ^^
#
# With the  first code,  if `opt`  has the value  'b' (set  by accident  or by
...
def g:ToggleSettingsAutoOpenFold(enable: bool) #{{{2
    ...
    # Make sure  our custom `[AOF]`  item is visible  in the status  line.  It
    # might not  be if  we press  `[oz` or `]oz`,  because then  our `submode`
    # plugin  is used  which  seems  to prevent  the  status  line from  being
...
#
# This is  because we run  `normal! zMzv` if, and  only if, the  foldlevel has
# changed, or if we get on a line in a closed fold.
#}}}
...
def MoveAndOpenFold(lhs: string, cnt: number)
    ...
    var scnt: string = cnt != 0 ? cnt->string() : ''
...
def DoesNotDistractInGoyo(): bool
    ...
    if !exists('#goyo') || &filetype == 'markdown'
        return true
    endif
...
def ToggleKittyLigatures(enable: bool)
    ...
    silent system('kitty @ disable-ligatures --all ' .. (enable ? 'always' : 'cursor'))
enddef
...
def AutoHlYankedText()
    ...
    silent! matchdelete(hl_yanked_text_id)
...
def Virtualedit(enable: bool) #{{{2
    if enable
        ...
        &l:virtualedit = get(w:, '_ORIG_OPTS', {})
            ->get('virtualedit', &l:virtualedit)
    endif
enddef
...
def NextCompiler(inc: number) #{{{2
    if !exists('b:compilers')
        echo 'b:compilers does not exist'
        return
    endif

    var idx: number = b:compilers
        ->index(&l:makeprg->matchstr('\S\+'))
    idx = (idx + inc) % len(b:compilers)
    var new_compiler: string = b:compilers[idx]
    execute $'compiler {new_compiler}'
    # useful when we press `coC`
    echomsg $'compiler {new_compiler}'
enddef

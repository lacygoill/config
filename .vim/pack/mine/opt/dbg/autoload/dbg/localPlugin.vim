...
export def Main(args: string) #{{{1
    var split_args: list<string> = split(args)
    var kind: string = args->matchstr('-kind\s\+\zs[^ -]\S*')
    var filetype: string = args->matchstr('-filetype\s\+\zs[^-]\S*')

    if split_args->index('-kind') == -1 || split_args->index('-filetype') == -1
        var usage: list<string> =<< trim END
            usage:
                DebugLocalPlugin -kind ftplugin -filetype bash
    ...
    if ['ftplugin', 'indent', 'syntax']->index(kind) == -1
...
export def Complete( #{{{1
    ...
    if from_dash_to_cursor =~ '^-kind\s*\S*$'
        var kinds: list<string> = ['ftplugin', 'indent', 'syntax']
        return kinds->join("\n")
    endif

    if empty(arglead) || arglead[0] == '-'

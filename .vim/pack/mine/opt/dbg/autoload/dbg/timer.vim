...
export def InfoOpen() #{{{1
    ...
    # elsewhere.
    #}}}
    # OK but why not re-capturing the info from `Populate()`?{{{
    #
    ...
    var tmp_file: string = tempname() .. '/timer_info'
    execute 'topleft :' .. (&columns / 3) .. ' vnew ' .. tmp_file
    &l:previewwindow = true
    &l:wrap = false
    wincmd p
enddef

export def Measure() #{{{1
    if date == []
        echomsg '  go!'
        date = reltime()
    else
        echomsg printf('%.3f seconds to do the task', reltime(date)->reltimefloat())
        date = []
    endif
enddef
var date: list<number>

export def Populate() #{{{1
    if infos == []
        infos = timer_info()
    endif

...
#        > 0 if x comes after y in buffer
    ...
    if ComparePos(y.start, x.start) <= 0 && ComparePos(y.end, x.end) >= 0
        return 'inner'
    endif
    if (ComparePos(x.start, y.end) <= 0 && ComparePos(y.start, x.end) <= 0)
...
def Highlight(exchange: dict<any>): any #{{{2
    ...
    if exchange.type == "\<C-V>"
        ...
        regions += range(exchange.start.line, exchange.end.line)
            ->map((_, v: number): list<number> => [v, blockstartcol, v, blockendcol])
    else
        var startcol: number
        var endcol: number
        if exchange.type == 'v'
            startcol = virtcol([exchange.start.line, exchange.start.column], true)[0]
            endcol = virtcol([exchange.end.line, exchange.end.column], true)[0]
        elseif exchange.type == 'V'
            startcol = 1
            endcol = virtcol([exchange.end.line, '$'])
        endif
        regions->add([exchange.start.line, startcol, exchange.end.line, endcol])
    endif
    return regions

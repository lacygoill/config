...
export def Title(): string
    ...
    if g:actual_curwin->str2nr() != win_getid()
        return pfx
    endif
    var len: number = strcharlen(w:quickfix_title)
    # Why not using `.80` in the outer `%{qf#...()}`?{{{
    #
    # When the  title is too long,  Vim would truncate its  start.  This would
    # include `[LL]`/`[QF]`.  I  want to always see the type  of the list (qfl
    # or ll), as well as the start of the command which produced it.
    #}}}
    # And why not using `%<` after the outer `%{qf#...()}`?{{{
    #
    # When the title is too long, Vim would truncate the line address.  I want
    # to always see where I am in the list.
    #}}}

...
def CreateReflinks(): dict<string> #{{{2
    ...
    var flags: string = 'cW'
    while search('\[.\{-}]\zs\%(\[\d\+]\|(\%(\%(https\=\|ftp\)://\|file:\).\{-})\)', flags) > 0 && id < GUARD
        flags = 'W'
        ...
        if char_under_cursor == '['
            var old_id: number = line
                ->matchstr('\%' .. col .. 'c\[\zs\d\+\ze]')
                ->str2nr()
...
def IsARealLink(): bool #{{{2
    return synstack('.', col('.'))
        ->indexof((_, id: number): bool => id->synIDattr('name') =~ '^markdownLink') >= 0
enddef
...
def MultiLineLinks(): bool #{{{2
    ...
    var flags: string = 'cW'
    var g: number = 0
    while search(pat, flags) > 0 && g <= GUARD | ++g
        flags = 'W'
        if IsARealLink()

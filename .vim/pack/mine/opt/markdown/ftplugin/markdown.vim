...
import autoload '../autoload/markdown/check.vim'
import autoload '../autoload/markdown/commitHash2link.vim'
import autoload '../autoload/markdown/fold/foldexpr.vim'
import autoload '../autoload/markdown/fold/howMany.vim'
import autoload '../autoload/markdown/fold/option.vim'
import autoload '../autoload/markdown/fold/promote.vim'
import autoload '../autoload/markdown/fold/put.vim'
import autoload '../autoload/markdown/fold/reorder.vim'
import autoload '../autoload/markdown/getDefinition.vim'
import autoload '../autoload/markdown/linkInline2ref.vim'
import autoload '../autoload/markdown/preview.vim'
import autoload '../autoload/markdown/window.vim'
import autoload 'fold/lazy.vim'
silent! import autoload 'bash.vim'
...
command -bar -buffer -nargs=1 -range=% -complete=custom,check.PunctuationComplete CheckPunctuation {
    # `:echo` is useful to erase the command from the command-line after its execution.
    echo check.Punctuation(<q-args>, <line1>, <line2>)
}
    #}}}

command -bar -buffer -nargs=? -range=% -complete=custom,commitHash2link.Completion CommitHash2Link {
    commitHash2link.Main(<line1>, <line2>, <q-args>)
}
...
command -bar -buffer -range=% FoldSortBySize reorder.BySize(<line1>, <line2>)
...
#     [ref]: link
#}}}
command -bar -buffer -range=% LinkInline2Ref linkInline2ref.Main()

# Mappings {{{1

nnoremap <buffer><nowait> <bar>p <ScriptCmd>preview.Main()<CR>

nnoremap <buffer><nowait> cof <ScriptCmd>foldexpr.Toggle()<CR>
...
nnoremap <Plug>(foldlevel-less) <ScriptCmd>option.FoldLevel('less')<CR>
nnoremap <Plug>(foldlevel-more) <ScriptCmd>option.FoldLevel('more')<CR>

try
    import autoload 'submode.vim'
    execute submode.Enter('foldlevel-more-or-less', 'n', 'br',
        '[of', '<Plug>(foldlevel-less)', (): number => &l:foldlevel)
    execute submode.Enter('foldlevel-more-or-less', 'n', 'br',
        ']of', '<Plug>(foldlevel-more)', (): number => &l:foldlevel)
...
nnoremap <buffer><nowait> gl <ScriptCmd>howMany.Print()<CR>

nnoremap <buffer><nowait> +[# <ScriptCmd>put.Main(false)<CR>
nnoremap <buffer><nowait> +]# <ScriptCmd>put.Main()<CR>

nnoremap <buffer><expr> +h markdown.ConvertToHelpLink()
...
xnoremap <buffer><expr><nowait> H promote.Main('less')
xnoremap <buffer><expr><nowait> L promote.Main('more')
...
# commentstring {{{2
# template for  a comment;  will be  used by `gc`  (could also  be used  by `zf`
# &friends, if `&l:foldmethod = 'manual'`)
...
# but not in a glossary which we sometimes open in a narrow window
if expand('%:t') == 'glossary.md'
    &l:wrap = false
endif
# }}}1
# Variables {{{1
...
if expand('%:p:h:h') == $HOME .. '/Wiki/linter'
    b:force_vim9_syntax = true
endif
...
if search('^````*\s*\w\+', 'n') > 0
    b:markdown_highlight = getline(1, '$')
        ->filter((_, v: string): bool => v =~ '^````*\s*\w\+')
        ->sort()
        ->uniq()
        ->map((_, v: string) => v->matchstr('````*\s*\zs\w\+'))
endif
...
# The default plugin uses the value 50.
# We increase it a little because we had issues with 50.
b:sync_setting = 'syntax sync minlines=70'

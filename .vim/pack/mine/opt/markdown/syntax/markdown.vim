...
import autoload '../autoload/markdown.vim'
...
#
# Edit: We need  `containedin`, for  example, to  allow `xCommentTitle`  to be
# contained in `xComment`.
#
# Edit: We don't use `markdownCommentTitle` anymore.  I found it too annoying.
#
# TODO:
#
...
# The variable does not exist when we open the TOC popup window.
if exists('b:sync_setting')
...
syntax match markdownQuotationMarks /“.\{-}”\|‘.\{-}’/
# }}}1
...
# Even on the next lines.  It can continue on a whole screen, until you insert
# the closing backtick.  This is distracting.
#}}}
...
#
# Some wiki page  may contain an embedded codeblock with  a language for which
# Vim has no  syntax plugin, like `fish` for example.   When that happens, the
# ending  ```` ```  ```` will  be wrongly  interpreted as  the beginning  of a
# fenced codeblock.
#
...
syntax region markdownFencedCodeBlock
    ...
    \ concealends
    \ display
    # `display`  is necessary  to avoid  collapsed highlighting  after a  long
    # fenced code block.
# }}}1
...
#    > to three spaces.
# }}}
#   We don't want to support more than 2 digits for the number.{{{
#
#     \ start=/^ \{,3\}\%([-*+]\|\d\{1,2}\.\)\s\+\S/
#                                ^------^
#
# First, it's highly unlikely you would ever write a list containing hundreds of
# entries.  Second, allowing more than that could cause a wrong highlighting:
#
#    > At the time of this writing, two Rust editions are available: Rust 2015 and Rust
#    > 2018.  This book is written using Rust 2018 edition idioms.
#    ^--^
#     this is not the number of a list item; this is just a date
#}}}
...
#     highlight default link markdownHideAnswer Ignore
#     highlight default link markdownShowAnswer PreProc
#}}}
...
# We  need to  match  a math  formula  to suppress  spurious  errors when  using
# something like `x_s` to represent "x index s".
syntax region markdownMathFormula start=/^\$\$/ end=/\$\$$/ oneline
# }}}1
...
# `\_[^]]*`: a newline and any other character, except a closing square bracket, as many as possible
...
#}}}
syntax region markdownLinkText
    ...
    \ keepend
    \ display
    # `display` is necessary to sometimes avoid collapsed highlighting.
...
highlight default link markdownBold Bold
highlight default link markdownItalic Italic
highlight default link markdownBoldItalic BoldItalic
highlight default link markdownQuotationMarks QuotationMarks

highlight default link markdownHeader                Title
highlight default link markdownHeaderItalic          TitleItalic
highlight default link markdownHeaderBold            TitleBold
highlight default link markdownHeaderBoldItalic      TitleBoldItalic
highlight default link markdownHeadingRule           markdownRule

highlight default link markdownFootnote              Typedef
highlight default link markdownFootnoteDefinition    Typedef
...
# `Typedef` is what the default markdown syntax plugin uses to highlight the id of a reference link.
highlight default link markdownIdDeclaration         Typedef
highlight default link markdownIdDelimiter           markdownLinkDelimiter
highlight default link markdownLinkRefTitle          String
# Why `Underlined`?{{{
#
...
# `Float` is what the default markdown syntax plugin uses to highlight a url.
highlight default link markdownUrl                   Float
highlight default link markdownUrlDelimiter          Function
highlight default link markdownUrlTitleDelimiter     Delimiter

highlight default link markdownIgnore                Ignore
...
#}}}
highlight default link markdownError                 Error
highlight default link markdownEscape                Special
highlight default link markdownTodo                  Todo

highlight default link markdownBlockquote            CommentBlockquote
highlight default link markdownBlockquoteBold        CommentBlockquoteBold
highlight default link markdownBlockquoteBoldItalic  CommentBlockquoteBoldItalic
highlight default link markdownBlockquoteCodeSpan    CommentBlockquoteCodeSpan
highlight default link markdownBlockquoteItalic      CommentBlockquoteItalic
highlight default link markdownCodeBlock             Comment
highlight default link markdownFencedCodeBlock       markdownCodeBlock
highlight default link markdownListItem              CommentListItem
highlight default link markdownListItemBlockquote    CommentListItemBlockquote
highlight default link markdownListItemBold          CommentListItemBold
highlight default link markdownListItemBoldItalic    CommentListItemBoldItalic
highlight default link markdownListItemCodeBlock     markdownCodeBlock
highlight default link markdownListItemCodeSpan      CommentListItemCodeSpan
highlight default link markdownListItemItalic        CommentListItemItalic
highlight default link markdownListItemOutput        markdownOutput
highlight default link markdownOption                CommentOption
highlight default link markdownOutput                PreProc
highlight default link markdownPointer               CommentPointer
highlight default link markdownRule                  CommentRule
highlight default link markdownTable                 CommentTable
# }}}1

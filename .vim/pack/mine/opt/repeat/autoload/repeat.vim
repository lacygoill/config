...
#
#     silent! repeat.Set("\<Plug>(my-map)", 3)
#
...
#     var regname = v:register
#     ...
#     # after all your other commands
#     silent! repeat.Setreg("\<Plug>(my-map)", regname)
...
#
#     silent! repeat.Set("\<Plug>(MyMap)", cnt)
#                                          ^^^
#
...
#     nnoremap <C-B> <ScriptCmd>Func()<CR>
#     def Func()
...
#     import autoload 'repeat.vim'
#     autocmd CursorMoved,TextChanged * #
#     nnoremap <C-B> xp<ScriptCmd>repeat.Set('<C-B>')<CR>
#     :% delete
#     'abc'->setline(1)
#     set cpoptions+=y
#     # press:  C-b
...
#}}}
nnoremap <unique> . <ScriptCmd>Dot(v:count)<CR>
...
#}}}
nnoremap <unique> u <ScriptCmd>Wrap('u', v:count)<CR>
nnoremap <unique> <C-R> <ScriptCmd>Wrap('<C-R>', v:count)<CR>
if maparg('U') == ''
    nnoremap <unique> U <ScriptCmd>Wrap('U', v:count)<CR>
endif
...
# Functions {{{1
# Interface {{{2
export def Set(sequence: string, count = -1) #{{{3
    ...
    augroup RepeatCustomMotion
        ...
        #
        # If `Set()` is invoked from operator-pending mode, then `b:changedtick`
        # is not incremented until the operator command has been fully executed:
        #
        #     omap <C-B> <ScriptCmd>Func()<CR>
        #     def Func()
        #         normal! l
        ...
        #     import autoload 'repeat.vim'
        #     autocmd CursorMoved,TextChanged * #
        #     onoremap <C-B> <ScriptCmd>Textobj()<CR>
        #     def Textobj()
...
export def Setreg(seq: string, name: string) #{{{3
    repeat.setreg = {seq: seq, name: name}
enddef

export def Invalidate(): string #{{{3
...
def Wrap(command: string, count: number) #{{{3
    ...
    var seq: string = (count != 0 ? count : '') .. command
    feedkeys(seq, 'in')
    ...
    #
    #     feedkeys((count != 0 ? count : '') .. command, 'in')
    #     →
    #     execute 'normal! ' .. (count != 0 ? count : '') .. command
    #
    ...
    if ticks_synchronized
        autocmd TextChanged <buffer> ++once {
            repeat.tick = b:changedtick

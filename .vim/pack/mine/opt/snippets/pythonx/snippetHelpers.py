# pylint: disable=anomalous-backslash-in-string,import-error
...
import re
import vim
# Commented because we don't need them ATM:
#
#     import random
#     import string

# TODO: understand all the new functions

# TODO: Split this file into several ones, once it becomes too big.
...
# This function will clean up first placeholder when this is empty.
    ...
    #                        ^-----------------------^
    #                        Don't remove anything if the 1st tabstop hasn't been emptied.
    #                        Alternative:
...
def complete(base, matches): #{{{1
    ...
    if base:
        ...
        #           ^----------^ ^--------------^ ^-------------------^{{{
        #                |              |         but keep only the ones which start with `base` (filtering)
        #                |              |
...
def make_context(snip): #{{{1
    ...
    #
    # See  first   line  in   `_make_jumper_jump()`.   I  think   this  function
    # is    useful   to    make   sure    `snip.context`   exists.     Otherwise
    # `'jumper' not in snip.context` would give an error.  Same thing for:
    #
    #     snip.context.update(...)
    #
    # In the past, we used this instead:
    #
    #     {'__dummy': None}
    #
    # Now,  we   prefer  to  use  `snip`   inside  the  value  to   suppress  an
    # `unused-argument` pylint warning.
    return {'_': snip}
...
def plugin_guard(snip): #{{{1
    ...
    finish = '\n' + vim.eval("repeat(' ', &l:shiftwidth)") + 'finish'
...
def trim_ws(snip): #{{{1
    ...
    # mode.  Besides,  it would remove the  indentation in front of  `$0` in the
    # `fu` snippet.
    ...
    # From `:help UltiSnips-buffer-proxy`:
    #
...
def undo_ftplugin(snip): #{{{1
    ...
    if '/ftplugin' in path_to_dir:
        ...
        # }}}
        # FIXME: When you  expand this snippet,  if you remove the  `set` line,{{{
        ...
        #
        # Edit: We could use this:
        #
        #     `!p snip.rv = ' ' if t[1] == '' else '|'`
        #
        # But what if we remove `:set ...` and `:unlet! ...`.
        # And what if we remove `:set ...`, and `:unlet! ...`, and `:execute 'autocmd! ...'`.
        # ...}}}
        ...
        anon_snip_body = (
            ...
            "\n    \ | execute 'autocmd! ${6:group_name} * <buffer>'}${7:"
            "\n    \ | execute '${8:n}unmap <buffer> ${9:lhs}'}${10:"
            "\n    \ | execute '${11:c}unabbrev <buffer> ${12:lhs}'}${13:"
            "\n    \ | delcommand ${14:Cmd}}"
            "\n    \\ \""
            '\n$0'
        )
        ...
        anon_snip_body = (
            '" Teardown {{' + '{1'
            + '\n'
            + "\nlet b:undo_indent = get(b:, 'undo_indent', 'execute')"
            + "\n    \ ..'| set ${1:indentkeys}<'"
            + '\n$0'
        )

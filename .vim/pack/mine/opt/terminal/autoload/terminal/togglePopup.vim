...
export def Main(): number #{{{2
    ...
    # Special Case: The last popup was closed by pressing `C-d`.
    # The shell process is  finished.  Wipe out the buffer so  that we start a
    # new shell.  Otherwise, we just get an unmodifiable terminal buffer which
    # is jarring.
    if bufexists(bufnr) && term_getjob(bufnr)->job_status() == 'dead'
        execute $'bwipeout {bufnr}'
    endif

    var opts: dict<any> = GetOpts()
    var d: dict<number> = Popup.Create(bufnr, opts)
    if d->empty()
        return 0
    endif
    ...
    #      doing it a second time would give `E716` (yes, even with a bang after `:unlet`)
    #}}}
...
def TerminalJobMapping() #{{{2
    ...
    execute printf(
        'tnoremap <buffer><nowait> %s <ScriptCmd>Main()<CR>',
        g:_termpopup_lhs
    )
enddef
...
def DynamicBorderColor(winid: number) #{{{2
    augroup DynamicBorderColor
        ...
        execute 'autocmd! ModeChanged nt:t ' .. cmd
        # Why inspecting `mode()`?{{{
        #
        ...
        # Afterward,   this  is   wrong;  we're   no  longer   automatically  in
        ...
        #}}}
        execute 'autocmd! ModeChanged t:nt '
...
def GetOpts(): dict<any> #{{{2
    ...
    return opts->extend({borderhighlight: OPTS.normal_highlight, term: true})
enddef

...
def WriteFilepaths(): string #{{{2
    ...
    if curfile == ''
        ...
        var argv: list<string> = argv()
        files = argv->map((_, v: string) => v->fnamemodify(':p'))
        # Special Case: We edit a file via the `+` shell argument:{{{
        #
        #     $ vim +'edit file'
        #
        # That's what we do in our fish function `omni-TUI-cheatkeys`, which runs:
        #
        #     $ editor +"edit ~/Wiki/cheatkeys/$pgm"
        #}}}
        if argv->index(curfile) == -1
            argv->add(curfile)
        endif
    endif
...
def OpenFiles(filelist: string) #{{{2
    ...
    try
        # Why the bang?{{{
        #
        # To suppress `E89` which is given when the current buffer is an unnamed one:
        #

...
# Interface {{{1
export def Rhs(lhs: string): string #{{{2
    # TODO: What's the purpose of this block?{{{
    #
    # In the past, it  has prevented us from jumping to the  start/end of a fold
    # in a terminal buffer captured by pressing `M-c` in tmux.
    # That's because of an interaction with `ShouldSkip()` in this script:
    #
    #     ~/.vim/pack/mine/opt/fold/autoload/fold/lazy.vim
    #
    # If the latter returns `true`, then `b:last_foldmethod` is removed.
    #
    # ---
    #
    # In any case, if  we really need to bail out early here,  at least it would
    # be nice to get a message.
    #
    # ---
    #
    # Edit: Commented because it prevents us  from jumping to the start/end of
    # a fold in a man buffer.  You could  fix this by adding this block as the
    # start of the aforementioned `ShouldSkip()`:
    #
    #     if &filetype == 'man'
    #         return false
    #     endif
    #
    # But let's first see if disabling this block causes any issue...
    #}}}
    ...
    #
    # `<ScriptCmd>`  handles `C-v`  just  like  it would  be  on a  command-line
    # entered with `:`. That is, it's  interpreted as “insert the next character
    # literally”.
    #
    # Solution: double `<C-v>`.
    #}}}
    ...
    #}}}
    return printf("%s%s\<ScriptCmd>%s(%s, %s, %d)\<CR>",
        ['v', 'V', "\<C-V>\<C-V>"]->index(mode) >= 0 ? "\<Esc>" : '',
        mode == 'no' ? 'V' : '',
        Jump,
        string(lhs),
        string(mode),
...
def NextFold(lhs: string)
    ...
    if lhs == '[z'
        ...
        if is_fold_start || (is_fold_end && is_next_line_foldable)

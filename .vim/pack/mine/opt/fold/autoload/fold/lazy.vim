...
# foldexpr.   That would  explain  why  the issue  gets  worse  as the  number
# of  lines  above  increases,  and  why  it  gets  worse  as  the  number  of
# inserted/removed characters increases.
#
# ---
#
...
#
# Besides, for a given buffer, we might be using:
#
...
#
# Solution: on `VimEnter` invoke `Compute()` in *all* windows
# (*in addition to installing the autocmds*):
#
#     for d: dict<any> in getwininfo()
#         win_execute(d.winid, 'Compute(false)')
#     endfor
#
# See:
...
#
# Suppose the script contains a bug  which can be reproduced when some options
# A  and B  are  both on,  or  both off.   But during  your  tests, you  can't
# reproduce with A  and B off, because  you use a small file.   You'll come to
# the conclusion that the bug is only triggered when A and B are on; you might
# find a solution, but you won't have fixed the bug in all cases.
#
# Also, you might  sometimes reproduce the issue, but not  always, because you
# don't know or have forgotten about the  fact that the number of lines in the
# file matters.
#
...
# commands so that they invoke `ComputeWindows()`.
#
...
# been done recently.
#}}}
# I want to test how our “lazyfold” feature behaves when `'foldmethod'` is set to `syntax`.  What should I do?{{{
#
# First,  temporarily remove  `rust` from  `STYLED_COMMENTS_FILETYPES` in  our
# vimrc; the latter would cause `'foldmethod'` to be reset to `marker`.
#
# Next, use `locate(1)` to find a rust file with a few hundreds lines.
#
# Finally, run:
#
#     $ vim --cmd 'let g:rust_fold=2' /path/to/file.rs
#}}}
#}}}

# Interface{{{1
export def ComputeWindows() #{{{2
    # compute folds in each window displaying the current buffer; not just the current window
    var curbuf: number = expand('<abuf>')->str2nr()
    for d: dict<any> in getwininfo()
        ...
        #     var curbuf: number = bufnr('<abuf>')->str2nr()
        #     for d: dict<any> in getwininfo()
        #         ->filter((_, v: dict<any>): bool => v.bufnr == curbuf)
        #         win_execute(d.winid, [
        #             'Compute(false)',
        ...
        # inactive window; you might want to preserve it.
        #
        # Besides, whether it's  an issue is debatable.  I mean,  the fold did
        # not exist in an inactive window, so when Vim closes it automatically
        # there, you can't  say that its state has not  been preserved; it did
        # not have a state to begin with.
        #}}}
        win_execute(d.winid, 'Compute(false)')
    endfor
enddef

export def Compute(noforce = true): string #{{{2
    ...
    #
    # When we call `Compute()` from a custom mapping, we don't pass the optional
    # argument, to let  the function know that it should  not recompute folds if
    # it has already been done recently.
    ...
    if exists('b:last_foldmethod') && &l:foldmethod == 'manual'
        ...
        if was_visible && foldclosed('.') >= 0
            # Do *not* move `normal! zv` in `ComputeWindows()`.{{{
            #
            ...
            # `fold#motion#Rhs()`).
            # If  you moved  `:normal! zv`  in `ComputeWindows()`,  it would  not be
            ...
            #     &l:foldmethod = b:last_foldmethod
            #
            # Btw,  the  reason  why  the  fold   seems  to  stay  open  is  because
            # `fold#motion#Rhs()`  runs  `:normal!  zv`  at the  end;  but  it  *is*
            # temporarily closed, and it *is* closed right before `]z` is pressed by
            # `fold#motion#Rhs()` (via `:normal`) for the first time in a new fold.
            #}}}
            # `:normal! zv` might be executed in an inactive window.{{{
            #
            # Which is ok.
    ...
    # Evaluate `foldlevel(1)` to make sure Vim recomputes folds, before we reset the foldmethod to manual. {{{
    #
    # Otherwise, there is a risk that no fold would be created:
    #
    #     setlocal foldminlines=0
    #     setlocal foldmethod=manual
    #     setlocal foldexpr=FoldExpr()

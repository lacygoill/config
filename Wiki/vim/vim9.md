...
IOW, your script doesn't need to start with `:vim9script`.
    ...
    $ tee /tmp/my_export.vim <<'EOF'
      vim9script
      export const MYCONST = 123
    EOF

    $ tee /tmp/my_import.vim <<'EOF'
...
Example:
    ...
    $ tee /tmp/test/import/foo.vim <<'EOF'
        vim9script
...
Example:
    ...
    $ tee /tmp/test/import/foo/bar/baz.vim <<'EOF'
        vim9script
...
Or a function name contains a `#`:
    ...
    $ tee /tmp/some/autoload/some.vim <<'EOF'
        vim9script
        def some#func()
        enddef
    EOF
...
In front of:
   ...
   - a function name if, and only if, its immediate *surrounding* context is
     Vim9 (the surrounding context of a function header is not the header
     itself)
...
was  sourced right  after the  script `122`  if, and  only if,  none of  them is
autoloaded (i.e. `/autoload/` is absent from their absolute paths).
...
Edit: Actually, they're not exactly equivalent:
```vim
vim9script
mkdir('/tmp/test', 'p')
cd /tmp/test
...
Edit: Not a  bug.  I think  it's just a consequence  of the rule  which requires
...
Edit: Actually, it's not just a matter of style.

...
Note that all these equalities remain true if you replace `array` with a pointer
to `array`.

---

For a 2-dimensional array, we also have these equalities:

    *array = *(&array[0]) = array[0] = &array[0][0]
    *array + N = address of (N + 1)th element (across subarrays)
    **array = *(&array[0][0]) = array[0][0]
    *(*(array + i) + j) = array[i][j]

The last equality can be explained like this:

                                             because array[i] is still a (sub)array
                                             v----------v
    *(array + i) = *(&array[i]) = array[i] = &array[i][0]
    *(array + i) + j = &array[i][0] + j = &array[i][j]
                       ^----------^

Again, all these equalities remain true if you replace `array` with a pointer to
`array`.

...
Same  thing for  an  octal/hexadecimal number,  except  `unsigned int` is  tried
between `int` and `long int`.
...
## floating-point types
    ...
    │ name        │ constant │ printf()     │ min      │ max      │
    ├─────────────┼──────────┼──────────────┼──────────┼──────────┤
    │ float       │ [fF]     │ %[efgaEFGA]  │ FLT_MIN  │ FLT_MAX  │
    ├─────────────┼──────────┼──────────────┼──────────┼──────────┤
    │ double      │ ∅        │ %[efgaEFGA]  │ DBL_MIN  │ DBL_MAX  │
    ├─────────────┼──────────┼──────────────┼──────────┼──────────┤
    │ long double │ [lL]     │ %L[efgaEFGA] │ LDBL_MIN │ LDBL_MAX │
    └─────────────┴──────────┴──────────────┴──────────┴──────────┘

To get the actual value, `#include <float.h>`, then `printf()` the variable with
the appropriate conversion specification (`%e`, `%le`, or `%Le` for `*_MIN`; not
`%f`).
...
About `FLT_DIG`: It's  a guarantee about  how many  digits survive a  round trip
through float storage without corruption.
...
`printf()` correctly gives us back the original float.
```c
 #include <stdio.h>
    int
main(void)
...
This time, the last digit is wrong, because `f` has more than `FLT_DIG` digits.

---

About  `FLT_MIN_10_EXP`: Its usual  value is  -37.  It  means that  a float  can
represent numbers as small as about 10^-37 before it starts running into trouble
representing tiny values accurately.
```c
 #include <stdio.h>
 #include <float.h>
...
main(void) {
    ...
    printf("ok_value  = %e\n", ok_value);
    printf("subnormal = %e\n", subnormal);
    printf("below_subnormal = %e\n", below_subnormal);

    return 0;
}
```
    FLT_MIN_10_EXP = -37
    ok_value  = 1.000000e-37
    subnormal = 1.401298e-45
    below_subnormal = 0.000000e+00

Notice how `subnormal` is not accurate; that's because we used an exponent lower
than `FLT_MIN_10_EXP`.
...
Note that  `printf()` uses  `%[fegFEG]` for `float`  *and* `double`  types.  But

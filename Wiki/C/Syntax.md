...
can't change  `array`, because that  would mean changing the  location (address)
where the array is stored.  You can use operations such as `array+1` to identify
the  next element  in an  array, but  `++array` is  not allowed.   The increment
operator  can be  used only  with the  names of  variables (or,  more generally,
...
(provided you didn't use `const` right before `ptr`).  For example:
    ...
    // make `ptr` points to the 1nd character of the string
    ptr++;
